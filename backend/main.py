...
def get_document_stats(
    ...
    document_id: str = Query(None, alias="document_id")  # Optional filter by document_id
):
...
async def get_field_stats(db: Session = Depends(get_db)):
    stats = (
        db.query(
            ...
            func.avg(Extraction.ocr_confidence).label('avg_document_ocr_confidence')
        )
        .filter(
            and_(
                or_(
...
def get_stats(db: Session = Depends(get_db)):
    ...
    correct_fields = db.query(Extraction).filter(Extraction.is_correct == True).count()
    return {
